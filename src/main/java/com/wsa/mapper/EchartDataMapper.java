...
public interface EchartDataMapper {
    ...
    @Select("SELECT " +
            ...
            "SUM(CASE WHEN DATE(u.registerTime) = DATE(DATE_SUB(CURDATE(), INTERVAL seq DAY)) THEN 1 ELSE 0 END) AS newUsers, " +
            "SUM(CASE WHEN DATE(u.lastLoginTime) = DATE(DATE_SUB(CURDATE(), INTERVAL seq DAY)) THEN 1 ELSE 0 END) AS activeUsers " +
            "FROM (SELECT 0 AS seq UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6) seqs " +
            "LEFT JOIN users u ON (DATE(u.registerTime) = DATE(DATE_SUB(CURDATE(), INTERVAL seqs.seq DAY)) " +
            "OR DATE(u.lastLoginTime) = DATE(DATE_SUB(CURDATE(), INTERVAL seqs.seq DAY))) " +
            "WHERE DATE(DATE_SUB(CURDATE(), INTERVAL seqs.seq DAY)) BETWEEN DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY) " +
